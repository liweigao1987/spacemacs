#shortcut for spacemacs

| emacs按键                             | 描述                                                                                                                   |
|:--------------------------------------|:-----------------------------------------------------------------------------------------------------------------------|
| leader t E e                          | 在holy和evil模式之间切换                                                                                               |
| C-\                                   | 切换输入法                                                                                                             |
| C-insert                              | 拷贝                                                                                                                   |
| S-insert                              | 粘贴                                                                                                                   |
| M-w ?                                 | easy-kill帮助                                                                                                          |
| C-x d                                 | 进入dired模式,可创建目录等操作                                                                                         |
| C-c C-w f                             | treemacs window, set default workspace                                                                                 |
| leader i k                            | 下方插入空行                                                                                                           |
| leader i j                            | 上方插入空行                                                                                                           |
| M-x M-n                               | 下一个输入命令记录                                                                                                     |
| M-x M-p                               | 上一个输入命令记录                                                                                                     |
| C-;                                   | 打开黏贴板                                                                                                             |
| leader F ...                          | frame操作                                                                                                              |
| M-.                                   | 查找定义                                                                                                               |
| M-?                                   | 查找引用                                                                                                               |
| M-y                                   | 列出kill-ring内容, n/p 切换条目                                                                                        |
| C-x z                                 | 重复前一条命令，之后可以只按z，不断重复                                                                                |
| leader s e                            | 进入、退出iedit-mode                                                                                                   |
| leader n r                            | 只显示当前函数，方便只在这一个函数上的操作或修改                                                                       |
| 输入框action                          | 执行命令弹出echo回显区的输入框后，如果右下提示C-c或者C-z说明可以执行action                                             |
| leader '                              | 弹出命令行界面                                                                                                         |
| M-s h .                               | 高亮光标所在位置的符号（不同颜色）                                                                                     |
| C-c C-l                               | 在snippet模式加载yasnippet模板                                                                                         |
| leader s o                            | overlay方式高亮显示当前符号                                                                                            |
| M-x list-faces-display                | customize face                                                                                                         |
| M-x list-colors-display               | scan colors                                                                                                            |
| M-x add-file-local-variable-prop-line | 在当前buffer中增加一个文件属性变量，例如：关闭当前文件的自动保存功能 variable: buffer-auto-save-file-name   value: nil |
| shift-enter                           | open file in other window                                                                                              |
| C-x ( commands C-x ) C-x e            | repeat commands                                                                                                        |
| C-u 数字 shortcut                     | 重复执行命令N次                                                                                                        |
| C-h l                                 | 列出最近100次按键操作                                                                                                  |
| C-u num 然后 M-x 命令                 | 给命令增加前缀参数执行                                                                                                                       |

| mark-ring   | 描述                                                                       |
|:------------|:---------------------------------------------------------------------------|
| leader r m  | 显示所有的mark ring条目（global和buffer）                                  |
| C-spc       | 设置mark并激活，意味着移动光标会高亮选择区域，进入global和buffer rings     |
| C-spc C-spc | 设置mark不激活，意味着不高亮选择区域，压入global和buffer rings，形成回溯栈 |
| C-x C-x     | 在激活的mark也就是高亮区域的头尾跳转                                       |
| C-u C-spc   | 在buffer内的ring里面循环回溯                                               |
| C-x C-spc   | 在global内的ring里面循环回溯，每个文件只保留一个最后设置的mark供回溯       |
| C-@         | 设置mark标记，可以%跳转或者移动光标多选                                    |
| M-@         | 增量标记单词                                                               |
| C-M-@/C-'       | 标记当前表达式                                                             |
| M-n         | 向前跳转                                                                   |
| M-p         | 向后跳转                                                                   |
| leader v v  | 递增式选中                                                                           |

| register         | 描述                                     |
| :---             | :---                                     |
| leader r r       | 显示所有的寄存器条目                     |
| C-x r spc 单字符 | 把当前位置压入《单字符》指定的寄存器     |
| C-x r j 单字符   | 跳转到《单字符》指定的寄存器的位置       |
| C-x r s 单字符   | 把mark的文本拷贝到《单字符》指定的寄存器 |
| C-x r i 单字符   | 把《单字符》寄存器的内容复制到光标位置   |
| C-x r + mark     | 把mark的文本追加的文本寄存器里面         |
|                  |                            |

| bookmark       | 描述 |
| :---           | :--- |
| C-x r m 字符串 | 设置bookmark,bookmark是持久化存储的     |

| kill-ring | 描述                                                                                    |
| :---      | :---                                                                                    |
| C-u 0 C-k | 删除该行光标前的部分                                                                    |
| C-k       | 删除该行光标后的部分                                                                    |
| C-d       | 删除字符，不进入ring. delete的一般是小范围的，不进入ring;kill的一般是大范围的，进入ring |
|           |                                                          |

| dired模式 | 描述                               |
| （        | 仅显示文件名列表                   |
| 0w        | 拷贝指定文件的绝对路径             |
| num C     | 拷贝光标位置及其后的num个文件      |
| d         | 设置删除标记                       |
| D         | 删除该文件                         |
| x         | 把带标记的文件删除                 |
| u         | 撤销删除标记                       |
| s         | 排序                               |
| R         | 重命名文件/移动文件                |
| Z         | 压缩或解压缩                       |
| =         | diff两个文件                       |
| M-=       | 比较backup文件                     |
| !         | 在文件上执行shell命令              |
| num m     | 批处理标记num个文件                |
| % m/d     | 正则表达式标记                     |
| +         | 创建目录                           |
| 标记 Q    | 批量查找替换，支持正则             |
| e         | 编辑文件                           |
| f         | 查找文件                           |
| g         | 刷新                               |
| G         | 改变组权限                         |
| k         | 隐藏该行                           |
| o         | 在另一个窗口中查找                 |
| C-o       | 在另一个窗口中查找，但不切换窗口   |
| q         | 退出dired模式                      |
| v         | 预览文件内容                       |
| M-del     | 去除所有标记                       |
| */C-u *   | 所有可执行文件加标记/去标记        |
| //C-u /   | 所有目录加标记/去标记              |
| M-}       | 移动到带有*或D标记的下一个文件位置 |
| M-{       | 移动到带有*或D标记的上一个文件位置 |
| >         | 移动到下一个目录                   |
| <         | 移动到上一个目录                   |
| C-x C-d   | 列出目录的文件清单                                   |
    
| treemacs                 | 描述                                             |
|:-------------------------|:-------------------------------------------------|
| ?/advanced-helpful-hydra | 按键提示                                         |
| P                        | peek,只看不切光标                                |
| o h/o l/o a a            | 在其他窗口打开文件                               |
| M-N/M-P                  | 配合peek,移动peek打开的buffer                    |
| :                        | 批量操作文件                                     |
| M                        | 标记/取消标记文件（标记后可以用:批量操作标记文件 |
| D                        | 批量删除文件                                     |

| evil按键                   | 描述                                    |
|:---------------------------|:----------------------------------------|
| C-[                        | evil退出编辑模式                        |
| spc w M window左上角的字母 | 切换窗口                                |
| .点号                      | evil mode重复执行前一次命令             |
| spc b . m-x                | 交换buffer内容                          |
| m-a                        | 光标到行首                              |
| m-e                        | 光标到行尾                              |
| m-w                        | 复制到kill-ring-buffer                  |
| m-m k p                    | 粘贴                                    |
| m-backspace                | 向前删除                                |
| tab                        | markdown模式自动对齐                    |
| {                          | 上移到函数头部                          |
| }                          | 下移到函数尾部                          |
| m 字母                     | 设置对应字母名的标记                    |
| ·（反引号） 字母           | 跳到字母标记的位置                      |
| ··                         | 跳到上次标记位置                        |
| ‘（单引号）字母           | 跳到字母标记的行首                      |
| C-o                        | 返回到上一次光标位置                    |
| C-l / z z                  | 当前行在可视窗口上中下位置显示          |

| git                      | 描述                          |
|:-------------------------|:------------------------------|
| magit-ediff-resolve-rest | 解决文件冲突                  |
| magit-status-mode        | git模式,支持多种git命令快捷键 |
| magit-reset-worktree     | 还原工程当前修改到head index  |
| magit-branch-checkout    | 切分支                        |

| minibuffer(helm界面最下方输入条，即M-x触发的窗口) | 描述                                     |
|:--------------------------------------------------|:-----------------------------------------|
| C-h m                                             | 显示minibuffer的介绍                     |
| C-j                                               | 显示当前命令的介绍（可以用来复制命令用） |
| C-o                                               | 切换不同的源组（分组）                   |
| F2/C-c o                                          | 在另一个窗口打开buffer                   |
| C-c C-o                                           | 在另一个frame打开buffer                  |
| C-space                                           | 标记某个条目                             |
| M-a                                               | 标记所有条目                             |
| M-i                                               | 多buffer搜索                             |
| C-M-v/V/y                                         | minibuffer下滚动其他窗口                 |
| C-h b                                             | key bindings                             |
| C-c C-e                                           | 进入edit模式                             |
| edit模式下 C-c C-q C-k                            | 删除所有                                 |
|                                                   |                                          |

| lsp          | 描述                                             |
|:-------------|:-------------------------------------------------|
| leader m r r | 基于语义重命名（变量或函数名），只修改语义范围的 |
| leader m g g | 跳转到符号定义                                   |
| leader m g r | 搜索该符号的引用                                 |
|              |                                                  |

| 搜索          | 描述                                                                                                                     |
|:--------------|:-------------------------------------------------------------------------------------------------------------------------|
| leader s d/D  | 在文件所在目录搜索符号，之后可以按C-l查找上级目录                                                                        |
| leader s P F3 | 把搜索结果保存到buffer里面，可以C-o逐个打开，类似occur,该action适用于所有搜索结果                                        |
| leader s f    | 在指定文件中搜索                                                                                                         |
| leader s s    | 在当前文件中搜索                                                                                                         |
| M-s o ...     | 打开occur搜索列表，ｏ, C-o                                                                                               |
| 批量编辑      | 搜索->C-c C-e->leader s e->C-c C-c                                                                                       |
| 转义字符      | \b该位置为空，例如\bcl\b匹配单词cl，或\<,\>                                                                              |
| 正则搜索      | spacemacs的所有搜索都支持输入正则表达式，比如test[0-9]+, .号是特殊字符，正则中表示匹配单个任意字符，当做字符用需要转义\. |
| C-M-s/r       | 增量正则搜索                                                                                                             |
| C-l           | 进入上级目录                                                                                                                         |
    
| 补全  | 描述                  |
|:------|:----------------------|
| M-/   | 触发自动补全          |
| C-M-i | 自动补全/自动修正拼写 |
| C-M-/ | 自动扩展拼写字符串    |

| 删除          | 描述                                                              |
|:--------------|:------------------------------------------------------------------|
| C-S-backspace | 删除整行                                                          |
| leader k d x  | 删除当前光标所在表达式，括号范围，进入evil-lisp-state状态         |
| leader k D x  | 删除光标前一个位置所在的表达式，括号范围，进入evil-lisp-state状态 |
| leader k esc  | 退出evil-lisp-state状态                                           |
| leader k w/W  | 在当前位置的表达式增加/删除括号                                   |
| M-d           | 向后删除单词                                                      |
| C-M-k         | 向前删除sexp, kill-sexp  Kill balanced expression forward         |
| C-M-backspace | 向后删除sexp                                                      |
| M-k           | 智能删除sexp，引号上带引号删除；引号内不带引号删除                |
|               |                                                                   |

| 选中    | 描述                               |
|:--------|:-----------------------------------|
| C-x h   | 全选                               |
| C-M-h   | 选中整个函数代码                   |
| M-h     | 选中以空行间隔的段                             |
| C-M-spc | 选中配对符号之间的段，包含配对符号 |
| C-]     | 选中表达式                         |

| 移动         | 描述                                                                         |
|:-------------|:-----------------------------------------------------------------------------|
| M-<          | 跳到顶部                                                                     |
| M->          | 跳到底部                                                                     |
| M-{          | 跳到段顶部                                                                   |
| M-}          | 跳到段底部                                                                   |
| C-M-f        | forward-sexp Move forward over a balanced expression                         |
| C-M-b        | backward-sexp  Move backward over a balanced expression                      |
| C-M-n        | forward-list  Move forward over a parenthetical group                        |
| C-M-p        | backward-list  Move backward over a parenthetical group                      |
| leader x K/J | 上/下移动行                                                                  |
| C-M-a        | 跳到函数开头                                                                 |
| C-M-e        | 跳到函数结尾                                                                 |
| leader j c   | 跳转到最近改动的位置                                                         |
| C-M-f/b/u/d  | 跳转到配对的符号，分为光标在配对符号处和配对符号内，配对符号内可以在内部跳转 |
| M-a          | 移动到c语句首                                                                |
| M-e          | 移动到c语句尾                                                                |
| M-q          | 自动选中                                                                     |
| <f5>         | 在匹配的括号间跳转                                                           |
| C-x </>      | 左右滑动窗口                                                                 |
|              |                                                                        |

| 复制         | 描述                     |
|:-------------|:-------------------------|
| leader f y y | 拷贝当前目录绝对路径     |
| leader f y n | 拷贝当前buffer的文件名   |
| leader f y D | 拷贝相对工程根目录的路径 |
| leader k y   | 复制配对的符号和内容                         |

| 注释            | 描述                                               |
|:----------------|:---------------------------------------------------|
| leader ; 数字 ; | 注释/反注释行，行数由数字决定，;;注释/反注释当前行 |
| M-;             | 跳到行尾加注释                                     |
| M-j             | 扩展为多行注释                                                   |

| layout       | 描述                           |
|:-------------|:-------------------------------|
| leader l 1-9 | 切换layout，支持多项目并行操作 |
| leader l d   | 删除layout                     |
|              |                              |

| 开关         | 描述               |
|:-------------|:-------------------|
| leader t TAB | 打开缩进列对齐符号 |

| org                   | 描述            |
|:----------------------|:----------------|
| org-agenda            | 搜索现有org文档 |
| org-capture           | 记录org文档     |
| M-x org-decrypt-entry | 解密当前org条目 |
| C-enter               | 新增标题列表    |
| M-enter               | 新增当前列表    |
| enter                 | 正文                |

| 窗口         | 描述                            |
|:-------------|:--------------------------------|
| leader w u/U | 回退窗口/撤销回退               |
| leader w w/W | 包含/不包含treemacs窗口切换窗口 |
| leader w M   | 交换buffer窗口                                |

| 编辑       | 描述             |
|:-----------|:-----------------|
| M-{/}      | 撤销到最初/最终  |
| q          | 当前点退出       |
| C-q        | 放弃             |
| C-t        | 前后字符互换位置 |
| M-t        | 前后单词互换位置 |
| C-x C-t    | 上下两行互换位置 |
| M-u/l      | 更改单词大小写   |
| leader f E | 进入sudo编辑                 |

| 正则表达式 | 描述                       |
|:-----------|:---------------------------|
| ^          | 匹配行首                   |
| $          | 匹配行尾                   |
| .          | 匹配任意单个字符           |
| .*         | 匹配任意个字符             |
| \<         | 匹配单词开头               |
| \>         | 匹配单词结尾               |
| []         | 匹配中括号中的任何一个字符 |
| \b         |                            |

| 时钟         | 描述           |
|:-------------|:---------------|
| display-time | 状态栏显示时间 |
|              |                |

| 日历        | 描述                  |
|:------------|:----------------------|
| calendar    | 显示近3个月日历       |
| C-f/C-b     | 按天移动              |
| C-n/C-p     | 按星期移动            |
| C-x </C-x > | 移动一个月            |
| C-v/M-v     | 按3个月移动           |
| M-}/M-{     | 按月移动              |
| C-x ]/C-x [ | 按年移动              |
| C-a /C-e    | 移动到星期的开始/结束 |
| M-a/M-e     | 移动到月的开始/结束   |
| M-</M->     | 移动到年的开始/结束   |
| g d         | 移动到指定日期        |
| .           | 移动到当前日期        |
| space       | 滚动另一个窗口        |
| a           | 显示近期节假日        |
| holidays    | 显示近期节假日        |
| x           | 节假日特殊显示        |
| h           | 显示光标位置日期是否为节假日                      |

| markdown           | 描述           |
|:-------------------|:---------------|
| markdown-view-mode | 浏览markdown文件 |

| undo       | 描述                                 |
|:-----------|:-------------------------------------|
| leader a u | undo tree                            |
| C-/        | undo                                 |
| C-?        | redo                                 |
| m后d       | 标记当前变更点，然后跟别的变更点diff |
| ?          | 快捷键提示窗口                       |

| 滚动视图 | 描述                   |
|:---------|:-----------------------|
| C-l      | 当前窗口上，中，下滚动 |
| C-M-S-l  | 其他窗口上，中，下滚动 |
| leader-N     | 滚动导航状态                                                                 |
| leader N k/j | 上/下滚动视图行                                                              |
| C-M-v/V      | 向下/上滚动另一个窗口                                                        |
|          |             |

| 矩形操作 | 描述                   |
|:---------|:-----------------------|
| C-x r t  | 在选定区域首列添加字符 |

| 多光标     | 描述                      |
|:-----------|:--------------------------|
| C->        | 标记下一行                |
| C-<        | 标记上一行                |
| leader t b | 打开multiple cursors mode |
|            |                         |
