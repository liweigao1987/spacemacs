...
makefile使用bear生成： bear -- 命令  例如：bear -- make all,一定要clean工程从头编译才是完整的包含所有源文件，否则可能不全
...
* spacemacs                                                      :tool:emacs:
1. .gitignore文件中忽略的文件或目录，不会被搜索。比如多仓库，如果忽略了别的仓库，将无法同时搜索忽略的仓库，此时就不能忽略这些目录
2. c使用ccls配置：如果compile_commands.json文件无法生成，可以使用gen_compile_commands.py先配置工程路径生成json文件，可以同时加上配置lsp-ccls-initialization-options，参考.spacemacs.ubuntu22文件

* clang                                                          :tool:clang:
1. 打印宏值
   #define _lwg_STR(x) #x
   #define lwg_STR(x) _lwg_STR(x)
   #pragma message("---------->test=" lwg_STR(test))
   ------------------------------------stop-compile-here------------------------------------
   _Static_assert(sizeof(char) == 1, "char 1"); //编译时判断条件
